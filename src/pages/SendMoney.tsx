...
/**
 * Simulates transfer processing.
 * Throws on invalid inputs; succeeds for valid transfers.
 */
function processTransfer(recipient: Recipient, amount: number): void {
  if (amount <= 0) {
    throw new Error(
      `Transfer failed: invalid amount \u20ac${amount.toFixed(2)}. Amount must be greater than zero.`
    );
  }

  if (!recipient.name || !recipient.accountInfo) {
    throw new Error(
      `Transfer failed: incomplete recipient information for ${recipient.name || 'unknown'}.`
    );
  }
